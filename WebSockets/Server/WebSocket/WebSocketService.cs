...
{
    public class WebSocketService : WebSocketBase, IServiceWebsocket
    {
        ...
        //Used to send commands to presistent connections, like a Websocket (virtual so it can be inherited and override in main class)
        public virtual void SendCommand(string sCommand, System.Collections.ArrayList aParameters) { }
        public virtual bool SendMobileDownload(int nMobileID, int UserID) { return false; }
        ...
        public virtual void Dispose()
        {
            ...
            if (_stream.CanWrite && !_isDisposed)
            {
                ...
                _isDisposed = true;
                _logger.Debug(this.GetType(), "Sent web socket close message to client");
        ...
        protected override void OnConnectionClose(byte[] payload)
        {
            ...
            _logger.Debug(this.GetType(), "Sent response close message to client");
            _isDisposed = true;
